...
class FormsFiveTest(TestCase):
    ...
    def test_text_input(self):
        ...
        class TextForm(forms.Form):
            text = forms.CharField(label='My Sample Text Input')
        ...
        self.assertTrue('>My Sample Text Input:</label>' in rendered, rendered)
        ...
        class TextForm(forms.Form):
            text = forms.CharField(placeholder='Test the Placeholder',
                widget=forms.TextInput()
            )

        rendered = TextForm(initial={'text': 'some initial text'}).as_p()
        self.assertTrue('placeholder="Test the Placeholder"' in rendered, rendered)
    ...
    def test_phone(self):
        ...
        class PhoneForm(forms.Form):
            tel = forms.CharField(widget=forms.TelephoneInput)
    ...
    def test_radio_select(self):
        ...
        self.assertTrue('checked="checked" type="radio" id="id_radio_2" value="fr" name="radio" required> Francais' in rendered, rendered)
    ...
    def test_datetimelocal_with_initial(self):
        """SplitDateTimeWidget with an initial value"""
        class DateTimeForm(forms.Form):
            dt = forms.DateTimeField(initial=datetime.datetime.now(),
                                     widget=forms.DateTimeLocalInput)

        rendered = DateTimeForm().as_p()
        self.assertTrue('value="' in rendered)
        self.assertTrue('type="datetime-local"' in rendered)

    def test_weekinput(self):
        """SplitDateTimeWidget with an initial value"""
        class DateTimeForm(forms.Form):
            dt = forms.DateTimeField(step=4, widget=forms.WeekInput)

        rendered = DateTimeForm().as_p()
        self.assertTrue('step="4"' in rendered)
        self.assertTrue('type="week"' in rendered)

    def test_monthinput(self):
        """SplitDateTimeWidget with an initial value"""
        class DateTimeForm(forms.Form):
            dt = forms.DateTimeField(widget=forms.MonthInput)

        rendered = DateTimeForm().as_p()
        self.assertTrue('type="month"' in rendered)
    ...
    def test_datalist(self):
        ...
        rendered = HockeyForm().as_p()
        self.assertTrue('value="' in rendered)
        self.assertTrue('list="hockey"' in rendered)
        self.assertTrue('<datalist' in rendered)
        self.assertTrue('Winnipeg</option>' in rendered)